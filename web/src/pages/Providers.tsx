    ...
    api
      ...
      .catch((e) => {
        if (
          ...
          'There are fabrics linked to this provider'
        ) {
    ...
    <Flex flexDir="column" as="main" flex={1} mt={4}>
      ...
      {deleteError && (
        <Alert w={750} mt={4} status="warning">
          <AlertIcon />
          <AlertTitle mr={2}>
            Não foi possível excluir este fornecedor!
          </AlertTitle>
          <AlertDescription>
            Ainda existem tecidos atrelados a ele.
          </AlertDescription>
          <CloseButton
      ...
      <Stack mt={4}>
        {providers?.map((provider) =>
          editingProviders.includes(provider.id) ? (
            <Flex
              key={provider.id}
              alignItems="center"
              width={600}
